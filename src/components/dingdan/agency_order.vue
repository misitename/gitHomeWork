    ...
    <el-card class="box-card">
      <div slot="header" class="clearfix">
        ...
        <div class="card-title">
          <div class="card-bar">
            <ul>
              <li v-for="(item,id) in barlist" :key="id" @click="active(item.id)" :class="{active:show==item.id}">{{item.title}}</li>
            </ul>
          </div>
      ...
      <el-table :data="tableData" style="width: 100%">
        ...
        <el-table-column prop="name" label="网址"></el-table-column>
        ...
        <el-table-column label="操作" width="320">
          ...
          <el-button type="success" size="mini" @click.native.prevent="check(scope.$index, tableData)">查看详情</el-button>
...
export default {
  ...
  // 查看详情
   check(index, rows) {
     this.$router.push("/agency_details");
   }
  }
};
</script>
